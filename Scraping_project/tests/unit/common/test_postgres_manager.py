...
class CursorStub:
    ...
    fetch_values: list[dict[str, Any]] | None = None

    def __post_init__(self):
        if self.executed is None:
            self.executed = []
        if self.fetch_values is None:
            self.fetch_values = []
...
def test_log_error_persists_record(patched_postgres):
    connection, *_ = patched_postgres
    mgr = PostgresManager(password='secret')
    mgr.log_error('stage1', 'http://example.com', 'Timeout', 'boom', http_status_code=504, retry_count=1)
