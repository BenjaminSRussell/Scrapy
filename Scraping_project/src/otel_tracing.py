...
Default is off / no-op when ``OTEL_EXPORTER_OTLP_ENDPOINT`` is unset so local
lean runs keep working. When set, spans export via OTLP to the collector
already defined in ``docker-compose.production.yml`` (gRPC ``4317`` /
HTTP ``4318``).
...
def init_tracing(service_name: str | None = None) -> bool:
    ...
    try:
        ...
        if protocol in ("http/protobuf", "http"):
            ...
            # gRPC exporter wants host:port (or http(s)://host:port)
        ...
        _tracer = trace.get_tracer("scrapy.otel", "1.0.0")
        _enabled = True
...
class OtelItemPipeline:
    ...
    the same correlation id. No-ops when the collector endpoint is unset.
    """
...
class OtelTracingExtension:
    ...
    def spider_closed(self, spider, reason: str) -> None:
        ...
        # Flush pending spans so a short crawl still shows up in Jaeger
        if _enabled:
            try:
                from opentelemetry import trace
