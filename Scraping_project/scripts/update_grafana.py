...
def save_dashboard(dashboard: dict, dashboard_path: Path):
...
def find_panel(dashboard: dict, panel_title: str = None, panel_id: int = None):
...
def add_panels(dashboard: dict, panel_specs: list):
...
def modify_panel(dashboard: dict, panel_spec: dict):
    """Modifies an existing panel."""
    logger.info("Modifying panel...")
    panel_title = panel_spec.get('title')
    panel_id = panel_spec.get('id')

    panel_to_modify = find_panel(dashboard, panel_title=panel_title, panel_id=panel_id)
...
def delete_panel(dashboard: dict, panel_spec: dict):
    """Deletes a panel from the dashboard."""
    logger.info("Deleting panel...")
    panel_title = panel_spec.get('title')
    panel_id = panel_spec.get('id')

    panel_to_delete = find_panel(dashboard, panel_title=panel_title, panel_id=panel_id)
...
def main():
    ...
    parser.add_argument("--op", required=True, choices=['add_panels', 'modify_panel', 'delete_panel'], help="Operation to perform.")
    parser.add_argument("--panel-spec", type=Path, required=True, help="Path to the JSON file with the panel specification.")
